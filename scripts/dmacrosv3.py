...
class DisciplinedAnnotations:
    ...
    def __init__(self, inputDir):
        ...
        self.expression = 0
        self.loc = 0
        self.undisciplinedlist = list()
        self.disciplinedlist = list()
    ...
    def __checkDiscipline__(self, treeifdefs, file):
        ...
        if (self.opts.all or self.opts.check & (1 << DisciplinedAnnotations.PATTLS)):
            ...
            if len(listdisciplined) != 0:
                self.disciplined += len(listdisciplined)
                self.disciplinedlist.extend(listdisciplined)
                #self.disciplinedlist.append([x for x in listdisciplined])
        ...
        self.undisciplinedlist = listundisciplined
    ...
    def checkFiles(self):
        ...
        # print self.results
        self.printDisciplinedAndUndisciplinedList()

##################################################
    def printDisciplinedAndUndisciplinedList(self):
        printDisciplined = list()
        printUndisciplined = list()
        print "[" + str(len(self.undisciplinedlist)) + "," + str(len(self.disciplinedlist)) + "]"
        for ifdefs in self.disciplinedlist:
            ifdef = ifdefs[0]
            endif = ifdefs[1]
            disciplined = lxml.etree.tostring(ifdef, method="text")
            nodeifdefsibs = [sib for sib in ifdef.itersiblings()]
            for sib in nodeifdefsibs:
                disciplined += lxml.etree.tostring(sib, method="text")
                if (sib == endif):
                    break
            #print "@D:"
            #print disciplined

            #for dis in printDisciplined:
            #    if disciplined in dis:
            #        printDisciplined.remove(dis)
            #printDisciplined.append(disciplined)
            printDisciplined.append(disciplined)

        for i in printDisciplined:
            for j in printDisciplined:
                if i in j and i is not j: #Is a substring and not equals?
                    printDisciplined.remove(j) #remove the bigger

        #Print
        #print("================")
        for disciplined in printDisciplined:
            print "@D:"
            print disciplined

        for ifdefs in self.undisciplinedlist:
            ifdef = ifdefs[0]
            endif = ifdefs[1]
            undisciplined = lxml.etree.tostring(ifdef, method="text")
            nodeifdefsibs = [sib for sib in ifdef.itersiblings()]
            for sib in nodeifdefsibs:
                undisciplined += lxml.etree.tostring(sib, method="text")
                if (sib == endif):
                    break
            #print "@U:"
            #print undisciplined
            printUndisciplined.append(undisciplined)

        for i in printUndisciplined:
            for j in printUndisciplined:
                if i in j and i is not j:  # Is a substring and not equals?
                    printUndisciplined.remove(j)  # remove the bigger
                elif j in i and i is not j:
                    printUndisciplined.remove(i)

        # Print
        #print("================")
        for undisciplined in printUndisciplined:
            print "@U:"
            print undisciplined

    def outputFileUndisciplinedList(self):
        countUndisciplined = 0
        for undisciplinedStr in self.undisciplinedlist:
            print "@:" + str(countUndisciplined)
            countUndisciplined += 1
            print undisciplinedStr
